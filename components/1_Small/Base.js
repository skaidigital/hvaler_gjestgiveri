...
export const Grid = ({ children, c }) => {
  const klasseNavn = `grid grid-cols-4 md:grid-cols-8 lg:grid-cols-12 gap-x-24 auto-rows-auto ${c}`;
...
export const FourSixFour = ({ c, children, id }) => {
  return (
    <div key={id} className={`${c} col-span-4 md:col-span-6 lg:col-span-4`}>
      {children}
    </div>
  );
};
...
export const FourSixEight = ({ c, children, id }) => {
  return (
    <div key={id} className={`${c} col-span-4 md:col-span-6 lg:col-span-8`}>
      {children}
    </div>
  );
};
